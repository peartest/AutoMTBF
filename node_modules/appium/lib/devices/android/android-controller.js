...
androidController.findElement = function (strategy, selector,type,cb) {
  console.log("[Debug][ZhaoJiajun]Enter find element")
  this.findUIElementOrElements(strategy, selector, type,false, "", cb);
};

androidController.findElements = function (strategy, selector, cb) {
  console.log("[Debug][ZhaoJiajun]Enter find elements")
  this.findUIElementOrElements(strategy, selector, 0 ,true, "", cb);
};

androidController.findUIElementOrElements = function (strategy, selector,type,many, context, cb) {
  console.log("[Debug][ZhaoJiajun]Enter findUIElementOrElements1,the type is " + type);
...
androidController.findElementFromElement = function (element, strategy, selector, cb) {
  console.log("[Debug][ZhaoJiajun]Enter findElementFromElement")
  this.findUIElementOrElements(strategy, selector, 0,false, element, cb);
};

androidController.findElementsFromElement = function (element, strategy, selector, cb) {
  console.log("[Debug][ZhaoJiajun]Enter findElementsFromElement")
  this.findUIElementOrElements(strategy, selector, 0,true, element, cb);
};
...
androidController.setValue = function (elementId, value, cb) {
  console.log("[Debug][ZhaoJiajun]The element id is:" + elementId);
  console.log("[Debug][ZhaoJiajun]The value is " + value);
  var params = {
    elementId: elementId,
    text: value,
    replace: false
...
androidController.getContentDesc = function(elementId,cb){
  var p = {elementId:elementId,attribute:"contentDesc"}
  this.proxy(["element:getAttribute",p],cb)
}
...
androidController.elementChecked = function (elementId,cb) {
  var p = {elementId: elementId, attribute: "checked"};
  this.proxy(["element:getAttribute", p], function (err, res) {
    if (err) return cb(err);
    var checked = res.value === 'true';
    cb(null, {
      status: status.codes.Success.code
      , value: checked
    });
...
androidController.elementCheckable = function (elementId,cb){
  var p = {elementId: elementId, attribute: "checkable"};
  this.proxy(["element:getAttribute", p], function (err, res) {
    if (err) return cb(err);
    var checkable = res.value === 'true';
    cb(null, {
      status: status.codes.Success.code
      , value: checkable
    });
...
androidController.elementClickable = function (elementId,cb){
  var p = {elementId: elementId, attribute: "clickable"};
  this.proxy(["element:getAttribute", p], function (err, res) {
    if (err) return cb(err);
    var clickable = res.value === 'true';
    cb(null, {
      status: status.codes.Success.code
      , value: clickable
    });
...
androidController.elementLongclickable = function (elementId,cb){
  var p = {elementId: elementId, attribute: "longClickable"};
  this.proxy(["element:getAttribute", p], function (err, res) {
    if (err) return cb(err);
    var longclickable = res.value === 'true';
    cb(null, {
      status: status.codes.Success.code
      , value: longclickable
    });
...
androidController.elementFocused = function (elementId,cb){
  var p = {elementId: elementId, attribute: "focused"};
  this.proxy(["element:getAttribute", p], function (err, res) {
    if (err) return cb(err);
    var focused = res.value === 'true';
    cb(null, {
      status: status.codes.Success.code
      , value: focused
    });
...
androidController.elementFocusable = function (elementId,cb){
  var p = {elementId: elementId, attribute: "focusable"};
  this.proxy(["element:getAttribute", p], function (err, res) {
    if (err) return cb(err);
    var focusable = res.value === 'true';
    cb(null, {
      status: status.codes.Success.code
      , value: focusable
    });
...
androidController.elementScrollable = function (elementId,cb){
  var p = {elementId: elementId, attribute: "scrollable"};
  this.proxy(["element:getAttribute", p], function (err, res) {
    if (err) return cb(err);
    var scrollable = res.value === 'true';
    cb(null, {
      status: status.codes.Success.code
      , value: scrollable
    });
...
androidController.forceStopApp = function(packageName,cb){
  this.adb.forceStop(packageName,function(err){
    if(err) return cb(err);
    cb(null,{
      status:status.codes.Success.code
    });
...
androidController.waitElementExist = function(resourceid,timeout,cb){
  logger.debug("Waits a specified length of time for a UI element to become visible.")
  var opts = {};
  if (resourceid) opts.elementId = resourceid;
  if (timeout) opts.timeout = timeout;
  this.proxy(["waitExist", opts], cb);
};

androidController.scrollElementToText = function(resourceid,text,instance,cb){
  logger.debug("Performs a swipe up on the UI element until the requested text is visible or until swipe attempts have been exhausted.")
  var opts = {};
  if (resourceid) opts.elementId = resourceid;
  ...
  this.proxy(["scrollToText", opts], cb);
}

androidController.clickElementWithDescription = function(description,type,cb){
  logger.debug("Click element with it's description")
  var opts = {};
  if (description) opts.elementDes= description;
  opts.type = type;
  this.proxy(["clickWithDes", opts], cb);
}

androidController.clickElementWithId = function(id,cb){
  logger.debug("Click element with it's id ")
  var opts = {};
  opts.id = id;
  this.proxy(["clickWithId",opts],cb)
}

androidController.captureScreenshotByUiautomator = function(storePath,storeName,cb){
  logger.debug("Take a screenshot by uiautomator");
  var opts = {};
  opts.storePath = storePath;
  opts.storeName = storeName;
  this.proxy(['captureByUiautomator',opts],cb);
}
...
androidController.drag = function (startX, startY, endX, endY, duration, touchCount, elementId, destElId, cb) {
  ...
  if (elementId !== null) {
    console.log("[ZHAOJIAJUN]elementId is not null");
...
androidController.screenCapture = function(filePath,fileName,cb){
  var screenCaptureCommand = null;
  if (this.args.udid){
    screenCaptureCommand = 'adb -s ' + this.args.udid + ' shell screencap -p ' + filePath + '/' + fileName;
  } else {
    screenCaptureCommand = ' adb shell screencap -p ' + filePath + '/' + fileName;
  }
  deviceCommon.screenCapture(screenCaptureCommand,filePath,fileName,this.args.udid,cb);
}

androidController.monkey = function(testPackage,cb){
  var monkeyCommand = 'adb shell monkey -p ' + testPackage + ' --pct-anyevent 0 --ignore-crashes --ignore-timeouts --ignore-security-exceptions --throttle 500 -v -v -v 2000'
  //var opts = {
  //  packageName: testPackage
  //};
  //this.proxy(["monkey",opts],cb);
  deviceCommon.monkey(monkeyCommand,cb);
}

androidController.pullFilesToLocal = function(dp,lp,cb){
  var pullCommand = null;
  if(this.args.udid){
    pullCommand = 'adb -s ' + this.args.udid + ' pull ' + dp + ' ' + lp;
  }
  else{
    pullCommand = 'adb pull ' + dp + ' ' + lp;
  }
  deviceCommon.pullFilesToLocal(pullCommand,cb);
}
...
androidController.performTouch = function (gestures, cb) {
  ...
  var doTouchDrag = function (gestures, cb) {
    console.log("[ZHAOJIAJUN]Do Touch Drag");
    var longPress = gestures[0];

    var elementId = longPress.options.element;
    if(elementId == null)
      console.log("[ZHAOJIAJUN]No element");
    ...
    this.getLocation(elementId, function (err, res) {
      ...
      var startY;
      if(elementId == null){
        startX = longPress.options.x;
        startY = longPress.options.y;
      }
      else{
        startX = res.value.x;
        startY = res.value.y;
      ...
      if (typeof destElId !== 'undefined' && destElId) {
        console.log("[ZHAOJIAJUN]cHECK destElId")
        ...
        console.log("[ZHAOJIAJUN]cHECK destElId,destElId undefined");
        if(elementId == null){
          return this.drag(startX, startY, endX, endY, 1, 1, null, destElId, cb);
        }
        return this.drag(startX, startY, endX, endY, 1, 1, elementId, destElId, cb);
      }
    }.bind(this));
...
// author:ZhaoJiajun,mail:Zhaojj0506@thundersoft.com
androidController.wakeUpDevice = function(cb){
  this.proxy(["wake"],cb);
};

// author:ZhaoJiajun,mail:Zhaojj0506@thundersoft.com
androidController.isScreenOn = function(cb){
  this.proxy(["isScreenOn"],cb);
}

// author:ZhaoJiajun,mail:Zhaojj0506@thundersoft.com
androidController.getCurrentPackage = function(cb){
  this.proxy(["currentPackage"],cb);
}

// author:ZhaoJiajun,mail:Zhaojj0506@thundersoft.com
androidController.startLogcat = function(wholeTestFolder,roundFolder,deviceid,scriptStartTime,scriptCurrentLoop,scriptName,logStorePath,cb){
  var opts = {
    wholeTestFolderName:wholeTestFolder,
    roundFolderName:roundFolder,
    deviceId:deviceid,
    scriptStartTime:scriptStartTime,
    scriptCurrentLoop:scriptCurrentLoop,
    scriptName:scriptName,
    logStorePath:logStorePath
  }
  this.proxy(["startLogcat",opts],cb);
}

// author:ZhaoJiajun,mail:Zhaojj0506@thundersoft.com
androidController.endLogcat = function(cb){
  this.proxy(["endLogcat"],cb);
}

// author:ZhaoJiajun,mail:Zhaojj0506@thundersoft.com
androidController.lockAndroidDevice = function(cb){
  this.proxy(["lock"],cb);
};

// author:ZhaoJiajun,mail:Zhaojj0506@thundersoft.com
androidController.clearText = function(element_resource_id,cb){
  var opts = {
    resourceid: element_resource_id
  };
  this.proxy(["element:cleartext",opts],cb);
};
