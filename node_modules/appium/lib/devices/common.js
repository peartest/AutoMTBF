...
exports.monkey = function(monkeyCommand,cb){
  logger.debug("Exe monkey with command: " + monkeyCommand);
  exec(monkeyCommand,{maxBuffer:524288},function(error,stdout,stderr){
    if(error != null){
      cb(new Error('Monkey test fail'));
    }
    else{
      cb(error, 'Successfully monkey test');
    }
  });
};

exports.screenCapture = function (screenCaptureCommand,screenshotPath,screenShotName,udid,cb){
  // mkdir for screenshot
  var makeDirCommand = null
  if(udid){
    makeDirCommand = 'adb -s ' + udid + ' shell mkdir ' + screenshotPath
  }
  else{
    makeDirCommand = 'adb shell mkdir ' + screenshotPath;
  }

  exec(makeDirCommand,function(error){
      });

  exec(screenCaptureCommand,function (error){
    if (error !== null) {
      cb(new Error('Unable to screen capture'));
    } else {
      cb(error, 'Successfully screen capture');
    }
  });
};

exports.pullFilesToLocal = function(pullCommand,cb){
  exec(pullCommand,function (error){
    if(error !== null){
      cb(new Error('pull files to local fail'));
    }
    else{
      cb(error,'pull files to local successfully')
    }
  })
}
