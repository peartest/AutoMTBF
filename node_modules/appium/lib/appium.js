...
Appium.prototype.configure = function (args, desiredCaps, cb) {
  ...
  try {
    ...
    if (!args.launch) {
      console.log("[ZHAOJIAJUN]LUNCH " + args.launch);
      console.log("[ZHAOJIAJUN]deviceType:" + deviceType);
      console.log("[ZHAOJIAJUN]enforceStringCaps:" + args.enforceStrictCaps);
  ...
  /*检查Caps中的内容，注意Android对象是继承了
  * 原始情况：
  * 如果Caps中的package为空，则将错误信息传给回调函数*/
...
Appium.prototype.invoke = function (cb) {
  ...
  /*注意：这里调整过，不再判断autoLaunch是否为False，注释掉的部分为原始代码*/
